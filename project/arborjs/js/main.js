      ...
      initMouseHandling:function(){
        ...
        var handler = {
          clicked:function(e){
            ...
            if('link' in dragged.node.data)
            {
                window.open(dragged.node.data.link);
            }
...
$.fn.arbor = function(options) { //定义插件的名称，这里为userCp
        var dft = {
            repulsion:1000,
            stiffness:600,
            friction:0.5,
        ...
        sys = arbor.ParticleSystem(ops.repulsion, ops.stiffness, ops.friction) // create the system with sensible repulsion/stiffness/friction
        sys.parameters({gravity:ops.gravity}) // use center-gravity to make the graph settle nicely (ymmv)
        sys.renderer = Renderer($(this));

        for (var n1 in ops.edges) {
          if (ops.edges.hasOwnProperty(n1)) {
            for (var n2 in ops.edges[n1]) {
              if (ops.edges[n1].hasOwnProperty(n2)) {
                var edgeWeight = ops.edges[n1][n2].weight;
                ops.nodes[n2].weight = ops.nodes[n2].weight ? ops.nodes[n2].weight + edgeWeight : edgeWeight;
              }
            }
          }
        }
        // select 3 nodes with the most weight.
        var max3 = [];
        for (var n in ops.nodes) {
          if (ops.nodes.hasOwnProperty(n)) {
            ops.nodes[n].weight || (ops.nodes[n].weight = 0);
            max3.push(ops.nodes[n]);
          }
        }
        ...
        var styles = ["biggest","bigger","bigger"];
        j = -1;
        for(var i=0;i<max3.length;i++)
        {
            if(i%2 == 0 && j<styles.length-1)
            {
               j++;
            }
            max3[0].style = styles[j];
        }
        // max3[0].style = "biggest";
        // max3[1].style = "bigger";
        // max3[2].style = "bigger";
